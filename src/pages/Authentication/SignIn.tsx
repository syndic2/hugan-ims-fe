...
import { AuthLoginErrors } from '../../data/auth/errors';
...
const SignIn: React.FC = () => {
  ...
  const { getAuthToken, saveAuth } = useContext(AuthContext) as AuthContextProps;

  const [isFetching, setIsFetching] = useState<boolean>(false);
  const [auth, setAuth] = useState<Auth>(Auth.create(InitialAuth));
  const [authErrors, setAuthErrors] = useState<AuthLoginErrors>();

  useEffect(() => {
    let path = '/';
    if (!getAuthToken()) path = '/auth/sign-in';

    navigate(path, { replace: true });
  }, []);

  const onChangeInput = useCallback((event: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    setAuth(prevState => Auth.create({
      ...prevState.props,
  ...
  const onClickLogin = useCallback(async () => {
    ...
    try {
      ...
      const { status, message, data } = await AuthService.login(AuthMapper.mapDomainToAuthLoginBody(auth));
      if (!status) {
        Swal.fire({
          ...SWAL_CONFIG,
          icon: 'error',
      ...
      saveAuth(Auth.create({ username: auth.username }), data?.token || '');
      navigate('/');
    } catch (error) {
      console.error(error);
    } finally {
    ...
    <div className="flex items-center justify-center h-screen">
      <div className="rounded-sm border border-stroke bg-white shadow-default dark:border-strokedark dark:bg-boxdark">
        <div className="flex flex-wrap items-center">
          ...
          <div className="relative w-full border-stroke dark:border-strokedark xl:w-1/2 xl:border-l-2">
